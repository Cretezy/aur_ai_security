...
struct Args {
    ...
    #[arg(long, env = "AUR_SECURITY_DATABASE", default_value = "sqlite.db")]
    database: PathBuf,
}
...
async fn main() -> Result<()> {
    init_logging();
    let args = Args::parse();
    info!(database = %args.database.display(), "starting AUR Security web server");
...
pub(crate) fn database(cx: &Cx) -> &dyn db::Database {
    app_context::<db::SqliteBackend>(cx)
}
